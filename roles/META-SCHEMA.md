...
(`homeserver.sh add` / `homeserver.sh remove`, landing in Phase 5+ of the
unified-CLI work). Adding or removing a service to a deployment
uses only this metadata — no per-service code lives in `homeserver.sh`.
...
|---|---|---|
| `display_name` | yes | Human-readable name shown in `homeserver.sh add` menu. |
...
| `playbooks` | yes | Ansible playbooks `homeserver.sh add` runs after inventory is updated, in order. |
| `on_remove` | yes | Reverse-side cleanup spec for `homeserver.sh remove`. |
...
`timezone`, `hostname`, `server_user`. Extend in `homeserver.sh` as new
sources are needed.
...
On `remove`, `homeserver.sh` reverses these (filters out the entries
this role added).
...
| `volumes_to_preserve_unless_purge` | List of rootless podman volume names that survive `remove` by default. `homeserver.sh remove <svc> --purge` deletes them too. |
...
5. That's it — `homeserver.sh add <name>` will pick it up via the meta file (once Phase 5 lands).

...
| `jukebox_squeezelite_name`     | `LMS-Player-{{ ansible_facts['hostname'] }}`| Player name shown in the LMS UI.                                                         |
| `jukebox_squeezelite_mac`      | `""` *(empty)*                              | Stable MAC address (LMS uses it as the player identity). Generated by `setup.sh`.        |

**Preset.** The squeezelite container ships a catalogue of presets
for specific DACs. `default` falls back to ALSA's default device,
which is fine for built-in audio but won't work in a VM without a
real DAC. Set the preset matching your hardware in your host_vars.

**MAC address.** LMS identifies players by MAC, so without a stable
value every restart registers as a new player (`SqueezeLite 2`,
`SqueezeLite 3`, …). `setup.sh` generates a locally-administered
unicast MAC (`02:xx:xx:xx:xx:xx`) on first install and writes it
into your host_vars as `jukebox_squeezelite_mac`. Leaving the
variable empty falls back to the container's auto-generated MAC,

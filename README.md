...
Being interested in owning my data and increasing my data privacy, I started reading and watching tutorials on individual topics, solving certain issues — like using Pi-hole to reduce ads. Combine Pi-hole with Unbound and your internet (or, more precisely, your DNS) provider can no longer track all of your digital activities.

In parallel I installed Paperless, because I always wanted a solution to manage and find my digital documents — they had been piling up in my mailbox or scattered across multiple file systems on my computer and elsewhere.

Photos on my iPhone pile up on local storage until I buy a new one. To get the migration done fast — because I want to use my new phone ;-) — I upload all photos to iCloud, which works great thanks to the Apple ecosystem. But besides the fact that I no longer own my data, over the years the iCloud bill gets bigger and bigger and so does the vendor lock-in. Switching to an "own your data" solution now *also* requires a data migration.

In parallel, Paperless works great in my home network — but I'd like to use the Paperless app on my iPhone from anywhere, to upload new PDFs or search them on the go. I watch a YouTube tutorial recommending I forward port 80 on my router to Paperless. But is that a good idea? Simply opening ports — and particularly port 80 — and forwarding *unencrypted* traffic to some service is not a good solution at all. And yes, my Paperless server is obviously still running over plain HTTP, because I used the default installation and skipped the effort of switching to HTTPS.

But now it's time to start digging. And yes, again there's a solution: you just need a reverse proxy and have to configure everything properly — great! At the same time, you find out there are plenty of open-source reverse-proxy solutions. More digging to figure out what's best for you. But hold on — if I use self-signed certificates, I need to install the certificate authority on each of my devices, including the ones family members use. Ugh. More digging… and what the heck, there is a solution: use Let's Encrypt certificates with the reverse proxy even for a server on your home network — great! Officially signed certs, no need to install anything on every device.

By the time I had solved everything to my needs, some time had passed, and several major Linux vulnerabilities had been disclosed in the meantime — and of course my default Linux installation wasn't configured to install security updates automatically. But yes, you just need to configure it ;-).

And ugh, there's still the backup topic on my to-do list — high priority — I need to find time for it… I hope I don't lose any data until then.

Oh, the Paperless team has been busy and meanwhile released several new versions. I hope it's just new features I don't need and nothing security-relevant I'm missing :-).
...
- **rsync** — large mutable trees where a full-volume tar would be wasteful (media, object-store buckets). A filesystem with snapshot capabilities on the NAS, like e.g. ZFS, is obviously benefitial.
...
| **Ente Photos** | Self-hosted photo & video library with iOS/Android apps (end-to-end encrypted). | <ul><li>`ghcr.io/ente-io/server:latest`</li><li>`ghcr.io/ente-io/web:latest`</li><li>`public.ecr.aws/docker/library/postgres:15`</li><li>`docker.io/dxflrs/garage:v1.0.1`</li></ul> | <ul><li>`entephoto-museum-config` — tar</li><li>`entephoto-garage-meta` — rsync</li><li>`entephoto-garage-data` — rsync</li><li>`ente_db` (Postgres) — pgdump</li></ul> |
